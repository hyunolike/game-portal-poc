...
게임 홈페이지 + 운영툴 백엔드 PoC (.NET 8 / SQL Server / Redis / GitHub Actions)
...
|---|---|---|
| 게임에 필요한 **웹사이트** 개발 | 공지 조회(분산 캐시), 쿠폰 사용, 내 쿠폰 내역 API | `src/GamePortal.Web.Api` |
...
tests/
  GamePortal.UnitTests         도메인 규칙, 검증기, 캐시 폴백 (47개)
  GamePortal.IntegrationTests  실제 SQL Server(Testcontainers) + WebApplicationFactory (23개)
```
